...
export function TitleBar({onSearch,onError,closeToTray}:{onSearch:()=>void;onError:(message:string)=>void;closeToTray:boolean}) {
 ...
 return <header className="window-bar">
  <div className="window-brand" data-tauri-drag-region><img className="brand-mark" src={brandMark} alt="" aria-hidden="true"/><strong>VeyDock</strong><span className="window-product">VDock</span></div>
